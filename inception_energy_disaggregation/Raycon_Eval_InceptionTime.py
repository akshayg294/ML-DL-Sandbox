...
def get_features(x):
    ...
    x['normalized_mains_power'] = 0  # Normalized = mains power - previous means
    ...
    for i in range(len(x)):
        ...
        if (label != 'no_event') and (activity_ongoing == False):
            ...
            x['time_since_last_event'].iloc[i] = (x['logged_on_utc'].iloc[
                                                      i] - prev_activity_time).microseconds / 1000000 + (
                                                         x['logged_on_utc'].iloc[i] - prev_activity_time).seconds
            x['time_since_event_start'].iloc[i] = (x['logged_on_utc'].iloc[
                                                       i] - current_activity_time).microseconds / 1000000 + (
                                                          x['logged_on_utc'].iloc[
                                                              i] - current_activity_time).seconds
...
def import_labels(property_id):
    ...
    labels['event_time'] = [datetime.datetime.fromtimestamp(timestamp / 1000) - datetime.timedelta(hours=5.5) for
                            timestamp in labels['event_time']]
    return labels
...
x_test_values = x_test[
    ['mains_power', 'prev_mean', 'time_since_last_event', 'time_since_event_start', 'normalized_mains_power']]
# %%
